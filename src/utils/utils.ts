...
export function getKgInfo({ kgId, kgSchema, backendUrl, searchParam = new URLSearchParams() }) {
  return fetch(`${backendUrl}/${encodeURIComponent(kgSchema)}/${kgId}?${searchParam.toString()}`).then(res => res.json())
}
